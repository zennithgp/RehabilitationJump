...
//		}
	...
	public void CheckHighScore(){
		gameStart = false;
		Debug.Log ("The current high score is: " + float.Parse (FileIOUtility.ReadStringFromFile (PATH + highScoreFile)));
		if (Mathf.Ceil(timer) < float.Parse (FileIOUtility.ReadStringFromFile (PATH + highScoreFile))) {
//			float tempInt = Mathf.Ceil (timer);
//			string newHighScore = tempInt.ToString();
			Debug.Log ("New high score! It's " + timer);
			float tempHiScore = Mathf.Ceil (timer);
			highScore = tempHiScore.ToString ();
//			FileIOUtility.WriteStringToFile (PATH + highScoreFile, timer.ToString(), false);
			FileIOUtility.WriteStringToFile (PATH + highScoreFile, highScore, false);
//			highScore = timer.ToString ();
	...
	public void SwitchLevel(){
		if (GetComponent<MidtermGameManager> ().levelFile == "level1.txt") {
			levelFile = "level2.txt";
			highScoreFile = "level2HighScore.txt";
			Debug.Log ("Switched to level 2.");

		} else if (GetComponent<MidtermGameManager> ().levelFile == "level2.txt") {
			levelFile = "level3.txt";
			highScoreFile = "level3HighScore.txt";
			Debug.Log ("Switched to level 3.");
		}else {
			levelFile = "level1.txt";
			highScoreFile = "level1HighScore.txt";
